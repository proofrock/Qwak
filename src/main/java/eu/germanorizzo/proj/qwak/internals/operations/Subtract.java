...
public class Subtract implements Operations.Performable {
    @Override
    public Operand perform(Operand... operands) throws EvalException {
        if (operands.length != 2)
            throw new EvalException("Operands for SUBTRACT must be 2, are " + operands.length);
            ...
            case NUM:
                switch (o2.getType()) {
                    case STRING:
                        throw new EvalException("Cannot SUBTRACT on strings");
                    case NUM:
                        return Operand.numItem(((BigDecimal) o1.getValue()).subtract((BigDecimal) o2.getValue()));
                    case BOOL:
                        throw new EvalException("Cannot SUBTRACT on booleans");
                    case NULL:
                        throw new EvalException("Cannot SUBTRACT on nulls");
                }
            case BOOL:
                throw new EvalException("Cannot SUBTRACT on booleans");
            case NULL:
        ...
        throw new EvalException("Invalid parameters combination for SUBTRACT");
    }
}
