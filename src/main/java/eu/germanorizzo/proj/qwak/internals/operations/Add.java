...
public class Add implements Operations.Performable {
    @Override
    public Operand perform(Operand... operands) throws EvalException {
        if (operands.length != 2)
            throw new EvalException("Operands for ADD must be 2, are " + operands.length);
        ...
        switch (o1.getType()) {
            case STRING:
                switch (o2.getType()) {
                    case STRING:
                        return Operand.strItem((String) o1.getValue() + (String) o2.getValue());
                    case NUM:
                        return Operand.strItem((String) o1.getValue() + ((BigDecimal) o2.getValue()).toString());
                    case BOOL:
            ...
            case NUM:
                switch (o2.getType()) {
                    case STRING:
                        return Operand.strItem(((BigDecimal) o1.getValue()).toString() + (String) o2.getValue());
                    case NUM:
                        return Operand.numItem(((BigDecimal) o1.getValue()).add((BigDecimal) o2.getValue()));
                    case BOOL:
                        throw new EvalException("Cannot ADD on booleans");
                    case NULL:
                        return o1;
