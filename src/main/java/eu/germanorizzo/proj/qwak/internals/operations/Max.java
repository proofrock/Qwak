...
public class Max implements Operations.Performable {
    @Override
    public Operand perform(Operand... operands) throws EvalException {
        if (operands.length != 2)
            throw new EvalException("Operands for MAX must be 2, are " + operands.length);
        ...
        switch (o1.getType()) {
            ...
            case BOOL:
                return o1 != o2 ? Operand.TRUE : o1;
        }

        throw new EvalException("Invalid parameters combination for MAX");
    }
}
